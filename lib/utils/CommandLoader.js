...
var commandLoader = module.exports = function(scope, customCommands){
     ...
     */
    this.load = function(types, options) {
        for(var i = 0; i < types.length; ++i) {
            this.loadType(this.scope,types[i], options);
        }
    };
     ...
     * @param  {Array}  types  types of commands
     */
    this.loadType = function(scope, type, options) {
        ...
        this.addQueueCommands(scope, commands, options);
    ...
    this.addQueueCommands = function(scope, commands, options) {
        ...
        for(var commandName in commands) {
            ...
            if( options && options.protocol &&
                excluded_list.indexOf( commandName ) == -1 ){
            ...
            scope[commandName] = this.addQueueItem(command, commandName);
        }
