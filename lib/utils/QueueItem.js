...
var QueueItem = module.exports = function(commandName, method, scope, args, delegate) {
    ...
    var last_arg = this.args.slice(-1)[0];
    var self = this;

    var done_and_next = function (){
      debug_log(self.queue.currentChildIndex + ' done');

      self.queue.currentChildIndex++;
      self.queue.next();
    };

    // hack callback
    if( typeof last_arg == 'function' ){
      this.args[this.args.length-1] = function( err, result ){
        var callback = last_arg;

        if( self.commandName === 'end' || self.commandName === 'call' ){
          debug_log(self.queue.currentChildIndex + ' done');

          if( self.commandName != 'end' ){
            self.queue.currentChildIndex++;
          }else{
            self.queue.did_end();
          }
    ...
    this.run = function (){
      debug_log( 'exec method ' + this.queue.currentChildIndex + ', name = ' + this.commandName );
    ...
    this.add = function ( item ){
      ...
      debug_log( 'add command ' + item.commandName );

      if( item.commandName === 'end' || item.commandName === 'call' ){
        debug_log( 'next' );
        debug_log( 'index = ' + this.currentChildIndex );
        debug_log( 'total = ' + this.children.length );
        this.next();
      }
    };
    ...
    this.did_end = function (){
      this.delegate.did_end();
    }
